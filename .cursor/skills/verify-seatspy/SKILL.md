...
Live commands share `~/.config/seatspy/cookies.txt`. Do not run two live drives at once. `scripts/run.py` holds `.verify/live.lock` for the duration of one command. If doctor reports an alive lock, stop.
