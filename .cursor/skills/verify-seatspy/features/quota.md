...
- `quota-expired` returns `SESSION_EXPIRED` and exit 1 when the homepage has an explicit `loggedIn: false` marker or its navigation includes `/auth/sign-in`.
...
- The live homepage currently omits `loggedIn`. Signed-in navigation has a `/user/account` path and omits `/auth/sign-in`. The parser normalizes absolute URLs, query strings, and trailing slashes before comparing those paths.
