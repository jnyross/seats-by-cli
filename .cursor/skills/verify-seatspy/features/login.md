...
- `~/.config/seatspy/op-service-account-token` exists, or desktop `op` is already signed in. Cloud agents get that file from the Cursor secret `OP_SERVICE_ACCOUNT_TOKEN` via `scripts/cloud-env.sh wire`.
